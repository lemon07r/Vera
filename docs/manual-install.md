...
- On CUDA backends, `vera setup --onnx-jina-cuda` and `vera repair --onnx-jina-cuda` refresh the downloaded runtime using the detected toolkit/runtime major version. On Linux Vera also checks CUDA runtime libraries (`LD_LIBRARY_PATH`, `ldconfig`, standard library dirs) when `CUDA_PATH` and `nvcc` are unavailable. If you switch between CUDA 12 and CUDA 13, rerun one of those commands.
